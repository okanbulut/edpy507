...
Responses to the items were recorded on a 7-point rating scale from 1 (completely disagree) to 7 (completely agree). However, to calculate total scores in the NFC Scale, the item responses must be recoded as -3 (completely disagree) to +3 (completely agree). @nfc-grass kindly shared their data files and other materials in an open repository: <https://osf.io/wn8xm/>. For the following analyses, we will use a subset of the original data including responses to the NFC Scale, demographic variables, and additional data related self-control etc. This dataset can be downloaded from [**here**](data_and_codes/nfc_data.csv). In addition, the R codes for the CTT analyses presented on this page are available [**here**](data_and_codes/ctt.R).

<br>

## Setting up R

In our example, we will conduct CTT-based analyses using the following R packages:
...
We can install all of the above packages by using the `install.packages()` command in R. Please remember that we only need to install these packages <u>once</u>. After that, we will be able to activate the packages in R and conduct CTT-based analysis.
...
Since the **QME** package [@R-QME] is available on [GitHub](https://github.com/zief0002/QME) instead of [CRAN](https://cran.r-project.org/), we will use the `install_github()` function from the **devtools** package to install it.
...
We can obtain a bit more information on the dataset using the `introduce()` and `plot_intro()` functions from the **DataExplorer** package:
...
The `describe()` function from the **psych** package is another useful function to get basic descriptive statistics for data collected for psychometric and psychology studies (see `?psych::describe` for more details about this function).
